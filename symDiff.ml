...
(*
(*returns the subexpression nested in a unary operator expression*)
let get_subexpr = function
  | Log e | Sin e | Cos e -> e
  | _ -> failwith "not a unary operator expression"

(*returns the subexpressions nested in a binary operator expression*)
let get_subexprs = function
  | Add (f,g) | Sub (f,g) | Mlt (f,g) | Div (f,g) | Pow (f,g) -> (f,g)
  | _ -> failwith "not a unary operator expression"

let fold_uop f_log f_sin f_cos e acc =
  let (Log f | Sin f | Cos f) =
*)
(*
type constructor =
  | C_Var
...
let get_constructor (e: expr) : constructor =
  match e with
  | Var -> C_Var
  | Const _ -> C_Const
  | Log _ -> C_Log
...
let fold_uop f_log f_sin f_cos e acc =
  let (Log e' | Sin e'| Cos e') = e in
  let f = match get_constructor e with C_Log -> f_log | C_
*)

(*
let fold_uop f_log f_sin f_cos e acc =
  let (Log e' | Sin e' | Cos e') = e
  and f = match e with |
  f_sel
  (match e with Log f -> f_log | Sin f -> f_sin | Cos f -> f_cos) f acc

let fold_bop f_add f_sub f_mlt f_div f_pow e acc1 acc2 =
  let selected_f =
    match e with
    | Add (f,g) -> f_add
*)

(*other ways to implement fold?*)
